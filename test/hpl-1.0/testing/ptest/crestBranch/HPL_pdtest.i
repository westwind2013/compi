...
# 125 "../HPL_pdtest.c"
   ...
   mat.nq = nq + 1;
# 153 "../HPL_pdtest.c"
   mat.ld = ( ( ( ( (1) > (mat.mp) ) ? (1) : (mat.mp) ) - 1 ) / ALGO->align ) * ALGO->align;
   do
   {
   ...
   info[0] = (vptr == ((void *)0)); info[1] = myrow; info[2] = mycol;
   (void) HPL_all_reduce( (void *)(info), 3, HPL_INT, HPL_max,
                          GRID->all_comm );
   if( info[0] != 0 )
   {
      if( ( myrow == 0 ) && ( mycol == 0 ) )
         HPL_pwarn( TEST->outfp, 170, "HPL_pdtest",
                    "[%d,%d] %s", info[1], info[2],
   ...
   mat.A = (double *)( ( ( (size_t)(vptr)+(((size_t)(ALGO->align) * sizeof(double) ))-1 ) / (((size_t)(ALGO->align) * sizeof(double) )) ) * (((size_t)(ALGO->align) * sizeof(double) )) )
                                                                          ;
   mat.X = mat.A + (mat.ld * mat.nq);
   HPL_pdmatgen( GRID, N, N+1, NB, mat.A, mat.ld, 100 );
# 191 "../HPL_pdtest.c"
...
# 307 "../HPL_pdtest.c"
   ...
   {
      if( ( myrow == 0 ) && ( mycol == 0 ) )
         HPL_pwarn( TEST->outfp, 315, "HPL_pdtest", "%s %d, %s",
   ...
   {
      cblas_dgemv( HplColumnMajor, HplNoTrans, mat.mp, nq, -1.0,
                 mat.A, mat.ld, mat.X, 1, 1.0, Bptr, 1 );
   }
   else if( nq > 0 )
   {
      cblas_dgemv( HplColumnMajor, HplNoTrans, mat.mp, nq, -1.0,
                 mat.A, mat.ld, mat.X, 1, 0.0, Bptr, 1 );
   }
   else { for( ii = 0; ii < mat.mp; ii++ ) Bptr[ii] = 0.0; }
   ...
   if( ( Anorm1 == 0.0 ) || ( AnormI == 0.0 ) ||
       ( Xnorm1 == 0.0 ) || ( Xnorm1 == 0.0 ) || ( N <= 0 ) )
   {
      resid1 = resid2 = resid3 = 0.0;
   }
   else
   {
   ...
   if( ( ( ( (resid1) > (resid2) ) ? (resid1) : (resid2) ) < TEST->thrsh ) &&
