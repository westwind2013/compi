# 1 "../HPL_ptimer.c"
# 1 "/home/westwind/mpibench/1hpl/hpl-1.0/testing/ptimer/crestBranch//"
# 1 "<built-in>"
# 1 "<command-line>"
...
static double HPL_ptimer_cpusec [64],
                   HPL_ptimer_cpustart [64],
                   HPL_ptimer_wallsec [64],
                   HPL_ptimer_wallstart[64];
...

{
   ...
   for( i = 0; i < 64; i++ )
   {
      HPL_ptimer_cpusec [i] = HPL_ptimer_wallsec [i] = 0.0;
      HPL_ptimer_cpustart[i] = HPL_ptimer_wallstart[i] = 5.0;
   }
...
# 316 "../HPL_ptimer.c"
   ...
   {
      if( HPL_ptimer_walltime() == -1.0 )
      { for( i = 0; i < N; i++ ) TIMES[i] = -1.0; return; }
      else
   ...
   for( i = 0; i < N; i++ ) TIMES[i] = ( ( (0.0) > (TIMES[i]) ) ? (0.0) : (TIMES[i]) );
