...
# 113 "../HPL_grid_init.c"
   ...
   ierr = MPI_Comm_split( COMM, ( rank < nprocs ? 0 : (-32766) ),
                          rank, &(GRID->all_comm) );
   if( GRID->all_comm == ((MPI_Comm)0x04000000) ) return( ierr );

   ierr = MPI_Comm_split( GRID->all_comm, myrow, mycol, &(GRID->row_comm) );
   if( ierr != 0 ) hplerr = ierr;
