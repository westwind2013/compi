# 1 "../HPL_reduce.c"
# 1 "/home/westwind/mpibench/1hpl/hpl-1.0/src/grid/crestBranch//"
# 1 "<built-in>"
# 1 "<command-line>"
...
# 111 "../HPL_reduce.c"
   MPI_Status status;
   void * buffer = ((void *)0);
   int hplerr=0, d=1, i, ip2=1, mask=0,
   ...
   if( ( mydist = ( ( (rank) < (ROOT) ) ? (size) + (rank) - (ROOT) : (rank) - (ROOT) ) ) == 0 )
   {
      do
      {
         mpierr = MPI_Recv( buffer, COUNT, ( ( DTYPE == HPL_INT ? ((MPI_Datatype)0x4c000405) : ((MPI_Datatype)0x4c00080b) ) ),
                            ( ( (ROOT) + (ip2) < (size) ) ? (ROOT) + (ip2) : (ROOT) + (ip2) - (size) ), tag, COMM,
                            &status );
         if( mpierr != 0 ) hplerr = mpierr;
      ...
      do
      {
         if( ( mydist & mask ) == 0 )
         {
            ...
            if( mydist & ip2 )
            {
               ...
               mpierr = MPI_Send( BUFFER, COUNT, ( ( DTYPE == HPL_INT ? ((MPI_Datatype)0x4c000405) : ((MPI_Datatype)0x4c00080b) ) ),
                                  partner, tag, COMM );
            }
