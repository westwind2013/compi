...
# 114 "../HPL_dlocmax.c"
   ...
   if( N > 0 )
   {
      A = ( (PANEL->A) + (II) + (JJ)*(PANEL->lda) );
      myrow = PANEL->grid->myrow;
      nprow = PANEL->grid->nprow;
      nb = PANEL->nb;
      kk = PANEL->ii + II + ( ilindx = cblas_idamax( N, A, 1 ) );
      { if( ( (0) >= 0 ) && ( (nprow) > 1 ) ) { if( (myrow) == (0) ) { if( (kk) < (nb) ) igindx = (kk); else igindx = (kk) + (nb)*((nprow)-1)*(((kk)-(nb))/(nb) + 1); } else if( (myrow) < (0) ) { igindx = (kk) + (nb) + (nb)*( ((nprow)-1)*((kk)/(nb)) + (myrow)-(0)-1+(nprow) ); } else { igindx = (kk) + (nb) + (nb)*( ((nprow)-1)*((kk)/(nb)) + (myrow)-(0)-1 ); } } else { igindx = (kk); } };
