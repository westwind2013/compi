...
# 167 "../HPL_pdpanrlT.c"
   ...
   while( Nm1 >= 1 )
   {
      Acur = ( (A) + (iip1) + (jj)*(lda) ); Anxt = ( (Acur) + (0) + (1)*(lda) );
      ...
      if( WORK[0] != 0.0 )
         cblas_dscal( Mm1, 1.0 / WORK[0], Acur, 1 );
      cblas_daxpy( Mm1, -(*(( (L1) + (jj+1) + (jj)*(n0) ))), Acur, 1, Anxt, 1 );
      HPL_dlocmax( PANEL, Mm1, iip1, jj+1, WORK );

      if( Nm1 > 1 )
      {
# 209 "../HPL_pdpanrlT.c"
         cblas_dger( HplColumnMajor, Mm1, Nm1-1, -1.0, Acur, 1,
                   ( (L1) + (jj+2) + (jj)*(n0) ), 1, ( (Anxt) + (0) + (1)*(lda) ),
                   lda );
