...
# 168 "../HPL_pdpancrN.c"
   ...
   while( Nm1 > 0 )
   {
      ...
      if( kk > 0 )
      {
         L1ptr = ( (L1) + (jj) + (jj+1)*(n0) );
# 200 "../HPL_pdpancrN.c"
         cblas_dgemv( HplColumnMajor, HplTrans, kk, Nm1, -1.0,
                    ( (L1) + (ICOFF) + (jj+1)*(n0) ), n0, ( (L1) + (jj) + (ICOFF)*(n0) )
                               , n0, 1.0, L1ptr, n0 );

         if( curr != 0 )
            cblas_dcopy( Nm1, L1ptr, n0, ( (A) + (ii) + (jj+1)*(lda) ), lda );
      }
...
# 233 "../HPL_pdpancrN.c"
      cblas_dgemv( HplColumnMajor, HplNoTrans, Mm1, kk+1, -1.0,
                 ( (A) + (iip1) + (ICOFF)*(lda) ), lda, ( (L1) + (ICOFF) + (jj+1)*(n0) )
                           , 1, 1.0, ( (A) + (iip1) + (jj+1)*(lda) ),
                 1 );
