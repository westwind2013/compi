...
# 167 "../HPL_pdpancrT.c"
   ...
   while( Nm1 > 0 )
   {
      ...
      if( kk > 0 )
      {
         L1ptr = ( (L1) + (jj+1) + (jj)*(n0) );
# 199 "../HPL_pdpancrT.c"
         cblas_dgemv( HplColumnMajor, HplNoTrans, Nm1, kk, -1.0,
                    ( (L1) + (jj+1) + (ICOFF)*(n0) ), n0, ( (L1) + (ICOFF) + (jj)*(n0) )
                            , 1, 1.0, L1ptr, 1 );

         if( curr != 0 )
            cblas_dcopy( Nm1, L1ptr, 1, ( (A) + (ii) + (jj+1)*(lda) ), lda );
      }
...
# 232 "../HPL_pdpancrT.c"
      cblas_dgemv( HplColumnMajor, HplNoTrans, Mm1, kk+1, -1.0,
                 ( (A) + (iip1) + (ICOFF)*(lda) ), lda, ( (L1) + (jj+1) + (ICOFF)*(n0) )
                     , n0, 1.0, ( (A) + (iip1) + (jj+1)*(lda) ), 1 );
