# 1 "../HPL_pdmxswp.c"
# 1 "/home/westwind/mpibench/1hpl/hpl-1.0/src/pfact/crestBranch//"
# 1 "<built-in>"
# 1 "<command-line>"
...
# 123 "../HPL_pdmxswp.c"
   ...
   mydist = ( ( (myrow) < (icurrow) ) ? (nprow) + (myrow) - (icurrow) : (myrow) - (icurrow) );
   ...
   {
      lda = PANEL->lda;
      cblas_dcopy( n0, ( (PANEL->A) + (II+(int)(WORK[1])) + (0)*(lda) ), lda,
                 Wmx, 1 );
      if( myrow == icurrow )
      { cblas_dcopy( n0, ( (PANEL->A) + (II) + (0)*(lda) ), lda, A0, 1 ); }
   }
   else { for( i = 0; i < n0; i++ ) Wmx[i] = 0.0; }
# 180 "../HPL_pdmxswp.c"
   if( ( Np2 != 0 ) &&
       ( ( partner = (int)((unsigned int)(mydist) ^ ip2 ) ) < nprow ) )
   {
      ...
      {
         if( mydist == (int)(ip2) )
            (void) HPL_sdrv( WORK, cnt_, 1001, A0, n0,
                             1001, ( ( (partner) + (icurrow) < (nprow) ) ? (partner) + (icurrow) : (partner) + (icurrow) - (nprow) )
                                             , comm );
         else
            (void) HPL_send( WORK, cnt_, ( ( (partner) + (icurrow) < (nprow) ) ? (partner) + (icurrow) : (partner) + (icurrow) - (nprow) )
                                    , 1001, comm );
      }
      else
      {
         if( mydist == 0 )
            (void) HPL_sdrv( A0, n0, 1001, Wwork, cnt_,
                             1001, ( ( (partner) + (icurrow) < (nprow) ) ? (partner) + (icurrow) : (partner) + (icurrow) - (nprow) )
                                             , comm );
         else
            (void) HPL_recv( Wwork, cnt_, ( ( (partner) + (icurrow) < (nprow) ) ? (partner) + (icurrow) : (partner) + (icurrow) - (nprow) )
                                    , 1001, comm );

         tmp1 = ( ( (Wwork[0]) < 0 ) ? -(Wwork[0]) : (Wwork[0]) ); gmax = ( ( (WORK[0]) < 0 ) ? -(WORK[0]) : (WORK[0]) );
         if( ( tmp1 > gmax ) ||
             ( ( tmp1 == gmax ) && ( Wwork[3] < WORK[3] ) ) )
         { cblas_dcopy( cnt_, Wwork, 1, WORK, 1 ); }
      }
   }
...
# 220 "../HPL_pdmxswp.c"
      ...
      while( k < hdim )
      {
         ...
         (void) HPL_sdrv( WORK, scnt, 1001, Wwork, rcnt,
                          1001, ( ( (partner) + (icurrow) < (nprow) ) ? (partner) + (icurrow) : (partner) + (icurrow) - (nprow) )
                                 , comm );
         ...
         if( ( tmp1 > gmax ) ||
             ( ( tmp1 == gmax ) && ( Wwork[3] < WORK[3] ) ) )
         {
            cblas_dcopy( ( rcnt == cnt0 ? cnt0 : cnt_ ), Wwork, 1,
                       WORK, 1 );
         }
         else if( rcnt == cnt0 )
         { cblas_dcopy( n0, Wwork+cnt_, 1, A0, 1 ); }
   ...
   else if( size_ > 1 )
   {
      ...
      mydis_ = ( ( (myrow) < (root) ) ? (nprow) + (myrow) - (root) : (myrow) - (root) );
      ...
      do
      {
         ...
         if( ( mydis_ & mask ) == 0 )
         {
            ...
            if( ( mydis_ & ip2_ ) != 0 )
            {
               (void) HPL_recv( A0, n0, ( ( (root) + (partner) < (nprow) ) ? (root) + (partner) : (root) + (partner) - (nprow) )
                                       , 1001, comm );
            }
            else if( partner < size_ )
            {
               (void) HPL_send( A0, n0, ( ( (root) + (partner) < (nprow) ) ? (root) + (partner) : (root) + (partner) - (nprow) )
                                       , 1001, comm );
            }
         }
         ip2_ >>= 1;
       ...
       ( ( partner = (int)((unsigned int)(mydist) ^ ip2 ) ) < nprow ) )
   {
      if( ( mydist & ip2 ) != 0 )
      {
         (void) HPL_recv( WORK, cnt_, ( ( (partner) + (icurrow) < (nprow) ) ? (partner) + (icurrow) : (partner) + (icurrow) - (nprow) )
                                 , 1001, comm );
      }
      else
      {
