...
# 167 "../HPL_pdpanllT.c"
   ...
   while( Nm1 > 0 )
   {
      ...
      L1ptr = ( (L1) + (jj+1) + (ICOFF)*(n0) ); kk = jj + 1 - ICOFF;
      cblas_dtrsv( HplColumnMajor, HplUpper, HplTrans, HplUnit, kk,
                 ( (L1) + (ICOFF) + (ICOFF)*(n0) ), n0, L1ptr, n0 );
      ...
      if( WORK[0] != 0.0 )
         cblas_dscal( Mm1, 1.0 / WORK[0], ( (A) + (iip1) + (jj)*(lda) ), 1 );
# 205 "../HPL_pdpanllT.c"
      cblas_dgemv( HplColumnMajor, HplNoTrans, Mm1, kk, -1.0,
                 ( (A) + (iip1) + (ICOFF)*(lda) ), lda, L1ptr, n0,
                 1.0, ( (A) + (iip1) + (jj+1)*(lda) ), 1 );

      HPL_dlocmax( PANEL, Mm1, iip1, jj+1, WORK );
      if( curr != 0 )
      {
         cblas_dcopy( kk, L1ptr, n0, ( (A) + (ICOFF) + (jj+1)*(lda) ), 1 );
