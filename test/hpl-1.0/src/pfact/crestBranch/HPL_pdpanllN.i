...
# 168 "../HPL_pdpanllN.c"
   ...
   while( Nm1 > 0 )
   {
      ...
      L1ptr = ( (L1) + (ICOFF) + (jj+1)*(n0) ); kk = jj + 1 - ICOFF;
      cblas_dtrsv( HplColumnMajor, HplLower, HplNoTrans, HplUnit, kk,
                 ( (L1) + (ICOFF) + (ICOFF)*(n0) ), n0, L1ptr, 1 );
...
# 206 "../HPL_pdpanllN.c"
      cblas_dgemv( HplColumnMajor, HplNoTrans, Mm1, kk, -1.0,
                 ( (A) + (iip1) + (ICOFF)*(lda) ), lda, L1ptr, 1,
                 1.0, ( (A) + (iip1) + (jj+1)*(lda) ), 1 );

      HPL_dlocmax( PANEL, Mm1, iip1, jj+1, WORK );
      if( curr != 0 )
      {
         cblas_dcopy( kk, L1ptr, 1, ( (A) + (ICOFF) + (jj+1)*(lda) ), 1 );
