...

{
   ...
   next = ( ((rank) != (size)-1) ? (rank) + 1 : 0 ); roo2 = ( ( size + 1 ) >> 1 );
   roo2 = ( ( (root) + (roo2) < (size) ) ? (root) + (roo2) : (root) + (roo2) - (size) );

   if( rank == root )
   {
      ierr = MPI_Send( PANEL->buffers[0], PANEL->counts[0], PANEL->dtypes[0], next, msgid, comm );

      if( ( ierr == 0 ) && ( size > 2 ) )
      {
         if( ( ((next) != (size)-1) ? (next) + 1 : 0 ) != roo2 )
         {
            ierr = MPI_Send( PANEL->buffers[0], PANEL->counts[0], PANEL->dtypes[0],
                             ( ((next) != (size)-1) ? (next) + 1 : 0 ), msgid, comm );
         }

         if( ierr == 0 )
         {
            ierr = MPI_Send( PANEL->buffers[0], PANEL->counts[0], PANEL->dtypes[0], roo2, msgid,
                             comm );
         }
      }
   }
   else
   {
      ...
      if( ( prev == root ) || ( rank == roo2 ) ||
          ( ( ((prev)!=0) ? (prev)-1 : (size)-1 ) == root ) ) partner = root;
      else partner = prev;
      ...
      if( ierr == 0 )
      {
         if( go != 0 )
         {
            ierr = MPI_Recv( PANEL->buffers[0], PANEL->counts[0], PANEL->dtypes[0], partner, msgid,
                             comm, &PANEL->status[0] );
            if( ( ierr == 0 ) && ( prev != root ) &&
                ( next != roo2 ) && ( next != root ) )
            {
               ierr = MPI_Send( PANEL->buffers[0], PANEL->counts[0], PANEL->dtypes[0], next, msgid,
                                comm );
            }
         }
         else { *IFLAG = 2; return( *IFLAG ); }
      }
   }
