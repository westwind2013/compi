...

{
   ...
   if( rank == root )
   {
      ierr = MPI_Send( PANEL->buffers[0], PANEL->counts[0], PANEL->dtypes[0], ( ((rank) != (size)-1) ? (rank) + 1 : 0 )
                             , msgid, comm );
   }
   else
   {
      ...
      if( ierr == 0 )
      {
         if( go != 0 )
         {
            ierr = MPI_Recv( PANEL->buffers[0], PANEL->counts[0], PANEL->dtypes[0], prev, msgid,
                             comm, &PANEL->status[0] );
            next = ( ((rank) != (size)-1) ? (rank) + 1 : 0 );
            if( ( ierr == 0 ) && ( next != root ) )
            {
               ierr = MPI_Send( PANEL->buffers[0], PANEL->counts[0], PANEL->dtypes[0], next,
                                msgid, comm );
            }
         }
