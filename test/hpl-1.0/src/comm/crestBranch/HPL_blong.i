...

{
   ...
   MPI_Comm comm;
   int COUNT, count, dummy=0, ierr=0,
   ...
   mydist2 = ( mydist = ( ( (rank) < (root) ) ? (size) + (rank) - (root) : (rank) - (root) ) ); indx = ip2;
   count = COUNT / size; count = ( ( (count) > (1) ) ? (count) : (1) );
   ...
   do
   {
      ...
      if( ( mydist & mask ) == 0 )
      {
         ...
         if( indx + ip2 < size ) { l = ip2 * count; lbuf = ( ( (lbuf) < (l) ) ? (lbuf) : (l) ); }
...
# 200 "../HPL_blong.c"
   ...
   prev = ( ((rank)!=0) ? (rank)-1 : (size)-1 ); next = ( ((rank) != (size)-1) ? (rank) + 1 : 0 );
   ...
   for( k = 0; k < npm1; k++ )
   {
      ...
      if( ( ( mydist + k ) & 1 ) != 0 )
      {
         ibufS = ( indx = ( ( (mydist) + (l) < (size) ) ? (mydist) + (l) : (mydist) + (l) - (size) ) ) * count;
         lbufS = ( indx == npm1 ? COUNT : ibufS + count );
         lbufS = ( ( (COUNT) < (lbufS) ) ? (COUNT) : (lbufS) ) - ibufS; lbufS = ( ( (0) > (lbufS) ) ? (0) : (lbufS) );

         ibufR = ( indx = ( ( (mydist) < (l+1) ) ? (size) + (mydist) - (l+1) : (mydist) - (l+1) ) ) * count;
      ...
      {
         ibufS = ( indx = ( ( (mydist) < (l) ) ? (size) + (mydist) - (l) : (mydist) - (l) ) ) * count;
         lbufS = ( indx == npm1 ? COUNT : ibufS + count );
         lbufS = ( ( (COUNT) < (lbufS) ) ? (COUNT) : (lbufS) ) - ibufS; lbufS = ( ( (0) > (lbufS) ) ? (0) : (lbufS) );

         ibufR = ( indx = ( ( (mydist) + (l+1) < (size) ) ? (mydist) + (l+1) : (mydist) + (l+1) - (size) ) ) * count;
         lbufR = ( indx == npm1 ? COUNT : ibufR + count );
         lbufR = ( ( (COUNT) < (lbufR) ) ? (COUNT) : (lbufR) ) - ibufR; lbufR = ( ( (0) > (lbufR) ) ? (0) : (lbufR) );
