# 1 "../HPL_blonM.c"
# 1 "/home/westwind/mpibench/1hpl/hpl-1.0/src/comm/crestBranch//"
# 1 "<built-in>"
# 1 "<command-line>"
...

{
   ...
   if( PANEL == ((void *)0) ) { return( 1 ); }
   if( PANEL->grid->npcol <= 1 ) { return( 1 ); }
# 89 "../HPL_blonM.c"
   return( 1 );
}
# 146 "../HPL_blonM.c"
...

{
   ...
   MPI_Comm comm;
   int COUNT, count, go=1, ierr=0, ibuf,
...
# 180 "../HPL_blonM.c"
   ...
   if( rank == root )
   {
      ...
      if( ierr == 0 )
         ierr = MPI_Ssend( PANEL->buffers[0], PANEL->counts[0], PANEL->dtypes[0],
                             ( ((rank) != (size)-1) ? (rank) + 1 : 0 ), msgid, comm );
...
# 208 "../HPL_blonM.c"
      if( ierr == 0 )
      {
         if( go != 0 )
         {
            ...
            ierr = HPL_packL( PANEL, 0, PANEL->len, 1 );

            if( ierr == 0 )
               ierr = MPI_Recv( PANEL->buffers[1], PANEL->counts[1], PANEL->dtypes[1],
   ...
   else mydist2 = ( mydist = ( ( (rank) < (root) ) ? (size) + (rank) - (root) : (rank) - (root) ) - 1 );

   indx = ip2; count = COUNT / npm1; count = ( ( (count) > (1) ) ? (count) : (1) );
   ...
   do
   {
      ...
      if( ( mydist & mask ) == 0 )
      {
         ...
         if( indx + ip2 < npm1 ) { l = ip2 * count; lbuf = ( ( (lbuf) < (l) ) ? (lbuf) : (l) ); }

         partner = mydist ^ ip2;

         if( ( mydist & ip2 ) != 0 )
         {
            partner = ( ( (root) + (partner) < (size) ) ? (root) + (partner) : (root) + (partner) - (size) );
            if( partner != root ) partner = ( ((partner) != (size)-1) ? (partner) + 1 : 0 );
# 278 "../HPL_blonM.c"
            if( lbuf > 0 )
            {

               if( ierr == 0 )
                  ierr = HPL_packL( PANEL, ibuf, lbuf, 1 );
            ...
            {
               if( ierr == 0 )
                  ierr = MPI_Recv( (void *)(&dummy), 0, ((MPI_Datatype)0x4c00010d), partner,
         ...
         else if( partner < npm1 )
         {
            ...
            if( lbuf > 0 )
            {
               ...
               if( ierr == 0 )
                  ierr = HPL_packL( PANEL, ibuf, lbuf, 0 );
            ...
            {
               if( ierr == 0 )
                  ierr = MPI_Ssend( (void *)(&dummy), 0, ((MPI_Datatype)0x4c00010d),
                                      partner, msgid, comm );
            }
         }
      }
   ...
   prev = ( ((rank)!=0) ? (rank)-1 : (size)-1 );
   if( ( ((prev)!=0) ? (prev)-1 : (size)-1 ) == root ) prev = root;
   next = ( ((rank) != (size)-1) ? (rank) + 1 : 0 );
   if( rank == root ) next = ( ((next) != (size)-1) ? (next) + 1 : 0 );
   ...
   for( k = 0; k < npm2; k++ )
   {
      ...
      if( ( ( mydist + k ) & 1 ) != 0 )
      {
         ibufS = ( indx = ( ( (mydist) + (l) < (npm1) ) ? (mydist) + (l) : (mydist) + (l) - (npm1) ) ) * count;
         lbufS = ( indx == npm2 ? COUNT : ibufS + count );
         lbufS = ( ( (COUNT) < (lbufS) ) ? (COUNT) : (lbufS) ) - ibufS; lbufS = ( ( (0) > (lbufS) ) ? (0) : (lbufS) );

         ibufR = ( indx = ( ( (mydist) < (l+1) ) ? (npm1) + (mydist) - (l+1) : (mydist) - (l+1) ) ) * count;
         lbufR = ( indx == npm2 ? COUNT : ibufR + count );
         lbufR = ( ( (COUNT) < (lbufR) ) ? (COUNT) : (lbufR) ) - ibufR; lbufR = ( ( (0) > (lbufR) ) ? (0) : (lbufR) );

         partner = prev;
      }
      else
      {
         ibufS = ( indx = ( ( (mydist) < (l) ) ? (npm1) + (mydist) - (l) : (mydist) - (l) ) ) * count;
         lbufS = ( indx == npm2 ? COUNT : ibufS + count );
         lbufS = ( ( (COUNT) < (lbufS) ) ? (COUNT) : (lbufS) ) - ibufS; lbufS = ( ( (0) > (lbufS) ) ? (0) : (lbufS) );

         ibufR = ( indx = ( ( (mydist) + (l+1) < (npm1) ) ? (mydist) + (l+1) : (mydist) + (l+1) - (npm1) ) ) * count;
         lbufR = ( indx == npm2 ? COUNT : ibufR + count );
         lbufR = ( ( (COUNT) < (lbufR) ) ? (COUNT) : (lbufR) ) - ibufR; lbufR = ( ( (0) > (lbufR) ) ? (0) : (lbufR) );
      ...
      if( lbufS > 0 )
      {
         ...
         if( ierr == 0 )
            ierr = HPL_packL( PANEL, ibufS, lbufS, 0 );

         if( ierr == 0 )
            ierr = MPI_Issend( PANEL->buffers[0], PANEL->counts[0],
      ...
      {
         if( ierr == 0 )
            ierr = MPI_Issend( (void *)(&dummy), 0, ((MPI_Datatype)0x4c00010d), partner,
      ...
      if( lbufR > 0 )
      {
         ...
         if( ierr == 0 )
            ierr = HPL_packL( PANEL, ibufR, lbufR, 1 );

         if( ierr == 0 )
            ierr = MPI_Recv( PANEL->buffers[1], PANEL->counts[1],
                               PANEL->dtypes[1], partner, msgid, comm,
                               &PANEL->status[0] );
