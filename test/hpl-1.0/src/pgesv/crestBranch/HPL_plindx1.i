...
# 163 "../HPL_plindx1.c"
   ...
   if( myrow == icurrow )
   {
      ...
      {
         src = IPID[i]; { if( ( (src) >= (nb) ) && ( (0) >= 0 ) && ( (nprow) > 1 ) ) { srcrow = (0) + 1 + ( (src)-(nb) ) / (nb); srcrow -= ( srcrow / (nprow) ) * (nprow); } else { srcrow = (0); } };

         if( srcrow == icurrow )
         {
            dst = IPID[i+1]; { if( ( (dst) >= (nb) ) && ( (0) >= 0 ) && ( (nprow) > 1 ) ) { dstrow = (0) + 1 + ( (dst)-(nb) ) / (nb); dstrow -= ( dstrow / (nprow) ) * (nprow); } else { dstrow = (0); } };

            { if( ( (src) < (nb) ) || ( (0) == -1 ) || ( (nprow) == 1 ) ) { il = (src); } else { int i__, j__; j__ = ( i__ = ( (src)-(nb) ) / (nb) ) / (nprow); il = (nb)*( j__ - i__ ) + ( (i__ + 1 - ( j__ + 1 ) * (nprow) ) ? (src) - (nb) : (src) ); } };
            LINDXA[ip] = il - iroff;
            ...
            {
               { if( ( (dst) < (nb) ) || ( (0) == -1 ) || ( (nprow) == 1 ) ) { il = (dst); } else { int i__, j__; j__ = ( i__ = ( (dst)-(nb) ) / (nb) ) / (nprow); il = (nb)*( j__ - i__ ) + ( (i__ + 1 - ( j__ + 1 ) * (nprow) ) ? (dst) - (nb) : (dst) ); } };
               LINDXAU[ip] = iroff - il;
            }
            ip++;
         }
