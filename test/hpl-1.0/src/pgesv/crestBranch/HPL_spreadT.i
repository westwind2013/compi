...
# 159 "../HPL_spreadT.c"
   ...
   int Cmsgid=1001, ibuf,
                             ierr=0, il, k, lbuf, lgth, myrow,
                             npm1, nprow, partner;
   ...
   if( SIDE == HplLeft )
   {
      ...
      do
      {
         ...
         if( ( mydist & mask ) == 0 )
         {
            lbuf = IPLEN[il+1] - ( ibuf = IPLEN[il-( ( (il) < ((int)(ip2)) ) ? (il) : ((int)(ip2)) )] );
...
# 219 "../HPL_spreadT.c"
                  if( ierr == 0 )
                     ierr = MPI_Recv( ( (U) + (0) + (ibuf)*(LDU) ), lbuf*N,
                                        ((MPI_Datatype)0x4c00080b), IPMAP[npm1-partner],
                                        Cmsgid, comm, &status );
...
# 248 "../HPL_spreadT.c"
                  if( ierr == 0 )
                     ierr = MPI_Send( ( (U) + (0) + (ibuf)*(LDU) ), lbuf*N,
                                        ((MPI_Datatype)0x4c00080b), IPMAP[npm1-partner],
                                        Cmsgid, comm );
...
# 318 "../HPL_spreadT.c"
                  if( ierr == 0 )
                     ierr = MPI_Recv( ( (U) + (0) + (ibuf)*(LDU) ), lbuf*N,
                                        ((MPI_Datatype)0x4c00080b), IPMAP[SRCDIST+partner],
                                        Cmsgid, comm, &status );
