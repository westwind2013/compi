...
# 300 "../HPL_pdupdateNN.c"
      while ( test == 2 )
      {
         ...
         cblas_dtrsm( HplColumnMajor, HplLeft, HplLower, HplNoTrans,
                    HplUnit, jb, nn, 1.0, L1ptr, jb, Uptr, jb );
         if( curr != 0 )
         {
# 323 "../HPL_pdupdateNN.c"
            cblas_dgemm( HplColumnMajor, HplNoTrans, HplNoTrans, mp, nn,
                       jb, -1.0, L2ptr, ldl2, Uptr, jb, 1.0,
                       ( (Aptr) + (jb) + (0)*(lda) ), lda );

            HPL_dlacpy( jb, nn, Uptr, jb, Aptr, lda );
         }
         else
         {
