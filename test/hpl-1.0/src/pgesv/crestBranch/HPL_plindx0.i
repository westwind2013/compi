...
# 236 "../HPL_plindx0.c"
   ...
   {
      src = IPID[i];
      { if( ( (src) >= (nb) ) && ( (0) >= 0 ) && ( (nprow) > 1 ) ) { srcrow = (0) + 1 + ( (src)-(nb) ) / (nb); srcrow -= ( srcrow / (nprow) ) * (nprow); } else { srcrow = (0); } }; LLEN[ srcrow ]++;
      ...
      if( myrow == srcrow )
      {
         ...
         if( myrow == icurrow )
         {
            { if( ( (dst) >= (nb) ) && ( (0) >= 0 ) && ( (nprow) > 1 ) ) { dstrow = (0) + 1 + ( (dst)-(nb) ) / (nb); dstrow -= ( dstrow / (nprow) ) * (nprow); } else { dstrow = (0); } };
            if( dstrow == icurrow )
            {
