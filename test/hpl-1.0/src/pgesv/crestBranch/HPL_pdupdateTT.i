...
# 164 "../HPL_pdupdateTT.c"
      ...
      while ( test == 2 )
      {
         nn = n - nq0; nn = ( ( (nb) < (nn) ) ? (nb) : (nn) );
# 181 "../HPL_pdupdateTT.c"
         HPL_dlaswp00N( jb, nn, Aptr, lda, ipiv );

         cblas_dtrsm( HplColumnMajor, HplLeft, HplUpper, HplTrans,
                    HplUnit, jb, nn, 1.0, L1ptr, jb, Aptr, lda );
# 200 "../HPL_pdupdateTT.c"
         cblas_dgemm( HplColumnMajor, HplNoTrans, HplNoTrans, mp, nn,
                    jb, -1.0, L2ptr, ldl2, Aptr, lda, 1.0,
                    ( (Aptr) + (jb) + (0)*(lda) ), lda );

         Aptr = ( (Aptr) + (0) + (nn)*(lda) ); nq0 += nn;
...
# 300 "../HPL_pdupdateTT.c"
      while ( test == 2 )
      {
         ...
         cblas_dtrsm( HplColumnMajor, HplRight, HplUpper, HplNoTrans,
                    HplUnit, nn, jb, 1.0, L1ptr, jb, Uptr, n );

         if( curr != 0 )
         {
# 324 "../HPL_pdupdateTT.c"
            cblas_dgemm( HplColumnMajor, HplNoTrans, HplTrans, mp, nn,
                       jb, -1.0, L2ptr, ldl2, Uptr, n, 1.0,
                       ( (Aptr) + (jb) + (0)*(lda) ), lda );

            HPL_dlatcpy( jb, nn, Uptr, n, Aptr, lda );
         }
         else
         {
...
# 429 "../HPL_pdupdateTT.c"
   }

   PANEL->A = ( (PANEL->A) + (0) + (n)*(lda) ); PANEL->nq -= n; PANEL->jj += n;




   if( PBCST != ((void *)0) ) *IFLAG = test;
