...
# 97 "../HPL_pdgesvK2.c"
   ...
   for( k = 0; k < depth; k++ )
   {
      ...
      icurcol = ( ((icurcol) != (npcol)-1) ? (icurcol) + 1 : 0 );
      tag = (( (tag)+1 > (3000) ? (2001) : (tag)+1 ));
   }



   HPL_pdpanel_new( GRID, ALGO, nn, nn+1, ( ( (nn) < (nb) ) ? (nn) : (nb) ), A, jstart,
   ...
   {
      jb = jstart - j; jb = ( ( (jb) < (nb) ) ? (jb) : (nb) ); j += jb;
      ...
      if( k < depth - 1 )
      {
         ...
         HPL_pdupdate( ((void *)0), ((void *)0), panel[k], nn );
      }
   }
   ...
   for( j = jstart; j < N; j += nb )
   {
      ...
      if( mycol == icurcol )
      {
         ...
         for( k = 0; k < depth; k++ )
            (void) HPL_pdupdate( ((void *)0), ((void *)0), panel[k], nn );
