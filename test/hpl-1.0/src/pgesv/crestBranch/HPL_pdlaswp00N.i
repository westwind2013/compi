   ...
   if( ( Np2 != 0 ) && ( ( partner = (int)(mydist ^ ip2) ) < nprow ) )
   {
      ...
      if( mydist == 0 )
      {
         (void) HPL_sdrv( U, usize, Cmsgid, W, llen[partner] * ldW,
                          Cmsgid, partner, comm );
         if( llen[partner] > 0 )
            HPL_dlaswp03N( llen[partner], n, U, jb, W, W+1, ldW );
      }
      else if( mydist == ip2 )
      {
...
# 312 "../HPL_pdlaswp00N.c"
         if( ( mydist >> (unsigned int)( k + 1 ) ) == 0 )
         {
            if( ( mydist >> (unsigned int)(k) ) == 0 )
            {
               (void) HPL_sdrv( U, usize, Cmsgid, ( (W) + (0) + (ipW)*(ldW) )
                                ...
                                partner, comm );
               HPL_dlaswp03N( llen[partner], n, U, jb, ( (W) + (0) + (ipW)*(ldW) )
                                   , ( (W) + (1) + (ipW)*(ldW) ), ldW );
               ipW += llen[partner];
            }
            else
            {
               (void) HPL_sdrv( W, llen[myrow]*ldW, Cmsgid, U, usize,
                                Cmsgid, partner, comm );
               HPL_dlaswp04N( ipA, llen[myrow], n, U, jb, A, lda, W,
   ...
   else
   {
      ...
      HPL_dlaswp05N( ipA, n, A, lda, U, jb, lindxA, lindxAU );
   }
