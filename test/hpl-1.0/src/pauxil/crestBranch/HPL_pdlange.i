# 1 "../HPL_pdlange.c"
# 1 "/home/westwind/mpibench/1hpl/hpl-1.0/src/pauxil/crestBranch//"
# 1 "<built-in>"
# 1 "<command-line>"
...
# 127 "../HPL_pdlange.c"
   ...
   { if( ( (0) >= 0 ) && ( (nprow) > 1 ) ) { int inb__, mydist__, n__, nblk__, quot__, src__; if( ( inb__ = (NB) - (0) ) <= 0 ) { nblk__ = (-inb__) / (NB) + 1; src__ = (0) + nblk__; src__ -= ( src__ / (nprow) ) * (nprow); inb__ += nblk__*(NB); if( ( n__ = (M) - inb__ ) <= 0 ) { if( (myrow) == src__ ) mp = (M); else mp = 0; } else { if( ( mydist__ = (myrow) - src__ ) < 0 ) mydist__ += (nprow); nblk__ = n__ / (NB) + 1; mydist__ -= nblk__ - (quot__ = (nblk__ / (nprow))) * (nprow); if( mydist__ < 0 ) { if( (myrow) != src__ ) mp = (NB) + (NB) * quot__; else mp = inb__ + (NB) * quot__; } else if( mydist__ > 0 ) { mp = (NB) * quot__; } else { if( (myrow) != src__ ) mp = n__ +(NB)+(NB)*(quot__ - nblk__); else mp = (M)+ (NB)*(quot__ - nblk__); } } } else { if( ( n__ = (M) - inb__ ) <= 0 ) { if( (myrow) == (0) ) mp = (M); else mp = 0; } else { if( ( mydist__ = (myrow) - (0) ) < 0 ) mydist__ += (nprow); nblk__ = n__ / (NB) + 1; mydist__ -= nblk__ - ( quot__ = (nblk__ / (nprow)) )*(nprow); if( mydist__ < 0 ) { if( (myrow) != (0) ) mp = (NB) + (NB) * quot__; else mp = inb__ + (NB) * quot__; } else if( mydist__ > 0 ) { mp = (NB) * quot__; } else { if( (myrow) != (0) ) mp = n__ +(NB)+(NB)*(quot__ - nblk__); else mp = (M)+ (NB)*(quot__ - nblk__); } } } } else { mp = (M); } };
   { if( ( (0) >= 0 ) && ( (npcol) > 1 ) ) { int inb__, mydist__, n__, nblk__, quot__, src__; if( ( inb__ = (NB) - (0) ) <= 0 ) { nblk__ = (-inb__) / (NB) + 1; src__ = (0) + nblk__; src__ -= ( src__ / (npcol) ) * (npcol); inb__ += nblk__*(NB); if( ( n__ = (N) - inb__ ) <= 0 ) { if( (mycol) == src__ ) nq = (N); else nq = 0; } else { if( ( mydist__ = (mycol) - src__ ) < 0 ) mydist__ += (npcol); nblk__ = n__ / (NB) + 1; mydist__ -= nblk__ - (quot__ = (nblk__ / (npcol))) * (npcol); if( mydist__ < 0 ) { if( (mycol) != src__ ) nq = (NB) + (NB) * quot__; else nq = inb__ + (NB) * quot__; } else if( mydist__ > 0 ) { nq = (NB) * quot__; } else { if( (mycol) != src__ ) nq = n__ +(NB)+(NB)*(quot__ - nblk__); else nq = (N)+ (NB)*(quot__ - nblk__); } } } else { if( ( n__ = (N) - inb__ ) <= 0 ) { if( (mycol) == (0) ) nq = (N); else nq = 0; } else { if( ( mydist__ = (mycol) - (0) ) < 0 ) mydist__ += (npcol); nblk__ = n__ / (NB) + 1; mydist__ -= nblk__ - ( quot__ = (nblk__ / (npcol)) )*(npcol); if( mydist__ < 0 ) { if( (mycol) != (0) ) nq = (NB) + (NB) * quot__; else nq = inb__ + (NB) * quot__; } else if( mydist__ > 0 ) { nq = (NB) * quot__; } else { if( (mycol) != (0) ) nq = n__ +(NB)+(NB)*(quot__ - nblk__); else nq = (N)+ (NB)*(quot__ - nblk__); } } } } else { nq = (N); } };

   if( ( ( (M) < (N) ) ? (M) : (N) ) == 0 ) { return( v0 ); }
   ...
   else if( NORM == HPL_NORM_1 )
   {
      ...
      if( nq > 0 )
      {
         ...
         if( myrow == 0 )
         { v0 = work[cblas_idamax( nq, work, 1 )]; v0 = ( ( (v0) < 0 ) ? -(v0) : (v0) ); }
         if( work ) free( work );
      }
   ...
   else if( NORM == HPL_NORM_I )
   {
      ...
      if( mp > 0 )
      {
         ...
         if( mycol == 0 )
         { v0 = work[cblas_idamax( mp, work, 1 )]; v0 = ( ( (v0) < 0 ) ? -(v0) : (v0) ); }
         if( work ) free( work );
      }
