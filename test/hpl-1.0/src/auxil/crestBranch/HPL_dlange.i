# 1 "../HPL_dlange.c"
# 1 "/home/westwind/mpibench/1hpl/hpl-1.0/src/auxil/crestBranch//"
# 1 "<built-in>"
# 1 "<command-line>"
...
# 114 "../HPL_dlange.c"
   ...
   if( NORM == HPL_NORM_A )
   {
      ...
      for( j = 0; j < N; j++ )
      {
         for( i = 0; i < M; i++ ) { v0 = ( ( (v0) > (( ( (*A) < 0 ) ? -(*A) : (*A) )) ) ? (v0) : (( ( (*A) < 0 ) ? -(*A) : (*A) )) ); A++; }
         A += LDA - M;
      }
   }
   else if( NORM == HPL_NORM_1 )
   {
      ...
      else
      {
         ...
         {
            s = 0.0;
            for( i = 0; i < M; i++ ) { s += ( ( (*A) < 0 ) ? -(*A) : (*A) ); A++; }
            work[j] = s; A += LDA - M;
         }



         v0 = work[cblas_idamax( N, work, 1 )]; v0 = ( ( (v0) < 0 ) ? -(v0) : (v0) );
         if( work ) free( work );
      }
   }
   else if( NORM == HPL_NORM_I )
   {
      ...
      else
      {
         ...
         for( j = 0; j < N; j++ )
         {
            for( i = 0; i < M; i++ ) { work[i] += ( ( (*A) < 0 ) ? -(*A) : (*A) ); A++; }
            A += LDA - M;
         }



         v0 = work[cblas_idamax( M, work, 1 )]; v0 = ( ( (v0) < 0 ) ? -(v0) : (v0) );
         if( work ) free( work );
      }
   }
