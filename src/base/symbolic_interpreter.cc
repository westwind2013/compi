...
value_t SymbolicInterpreter::NewInputRank(type_t type, addr_t addr) {
	...
	// record the indices of variables indicating MPI ranks (MPI_COMM_WORLD) 
	if (find(symbolic_ranks.begin(), symbolic_ranks.end(), 
		num_inputs_) == symbolic_ranks.end()) 
		symbolic_ranks.push_back(num_inputs_);
