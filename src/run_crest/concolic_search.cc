...
Search::Search(const string& program, int max_iterations, int rank_num,
		int rank_id, int num_params) :
		program_(program), max_iters_(max_iterations), num_iters_(0), rank_num_(
				rank_num), rank_id_(rank_id), num_params_(num_params) {
...
void Search::LaunchProgram(const vector<value_t>& inputs) {
	...
	if (0 != rank_id_) {
		command += "mpirun -np " + std::to_string(rank_id_) + " "
				+ program_clean + " " + inputs_str + " : -np 1 " + program_;
		if (rank_id_ + 1 < rank_num_) {
			command += " : -np " + std::to_string(rank_num_ - rank_id_ - 1)
					+ " " + program_clean + " " + inputs_str;
		}
	} else {
		command += "mpirun -np 1 " + program_ + " : -np "
				+ std::to_string(rank_num_ - 1) + " " + program_clean + " "
				+ inputs_str;
	}
...
BoundedDepthFirstSearch::BoundedDepthFirstSearch(const string& program,
		int max_iterations, int rank_num, int rank_id, int num_params,
		int max_depth) :
		Search(program, max_iterations, rank_num, rank_id, num_params), max_depth_(
				max_depth) {
}
...
RandomInputSearch::RandomInputSearch(const string& program, int max_iterations,
		int rank_num, int rank_id, int num_params) :
		Search(program, max_iterations, rank_num, rank_id, num_params) {
}
...
HybridSearch::HybridSearch(const string& program, int max_iterations,
		int rank_num, int rank_id, int num_params, int step_size) :
		Search(program, max_iterations, rank_num, rank_id, num_params), step_size_(
				step_size) {
}
...
CfgHeuristicSearch::CfgHeuristicSearch(const string& program,
		int max_iterations, int rank_num, int rank_id, int num_params) :
		Search(program, max_iterations, rank_num, rank_id, num_params), cfg_(
